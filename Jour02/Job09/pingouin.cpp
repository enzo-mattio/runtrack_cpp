...
#include <algorithm>
#include <set>
#include <unordered_map> // Ajout de l'en-tête pour unordered_map
...
public:
    ...
    std::unordered_map<std::string, std::string> journal; // unordered_map pour le journal
    ...
    Pingouin(const Pingouin &autre) 
        ...
        nom(autre.nom), competences(autre.competences), journal(autre.journal) {}
...
int main() {
    ...
    // Ajouter des entrées dans le journal de chaque pingouin
    for (const auto& pingouinPtr : Pingouin::colony) {
    ...
    for (const auto& pingouinPtr : Pingouin::colony) {
        ...
        pingouinPtr->afficherJournal(); // Afficher le journal de chaque pingouin
        std::cout << std::endl;
    }
