...
#include <unordered_set> // Ajout de l'en-tête pour unordered_set
...
public:
    ...
    std::unordered_set<std::string> emplacementsTresors; // Ajout de l'unordered_set pour les emplacements de trésors
    ...
    // Fonction pour ajouter un emplacement de trésor
    void ajouterEmplacementTresor(const std::string& emplacement) {
        emplacementsTresors.insert(emplacement);
    }

    // Fonction pour afficher les emplacements de trésors
...
int main() {
    ...
    for (const auto& pingouinPtr : Pingouin::colony) {
        ...
        pingouinPtr->ajouterEmplacementTresor("Ile mystérieuse");
